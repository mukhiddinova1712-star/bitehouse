      ...
      <div className="container-custom relative z-10">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-gold-500/30 bg-gold-500/5 mb-4">
            <span className="text-gold-500 text-xs font-sans tracking-[0.2em] uppercase">
              {t.gallery.badge}
            </span>
          </div>
        ...
        <div className="flex justify-center gap-3 mb-10 flex-wrap">
          {tabs.map(tab => (
            <button
              ...
              className={`px-6 py-2.5 rounded-full text-xs font-sans font-semibold tracking-wider uppercase transition-all duration-300
                ${activeTab === tab
          ...
          >
            {filtered.map((item, i) => (
              <motion.div
                ...
                className="relative rounded-xl overflow-hidden cursor-pointer group h-52 bg-gradient-to-br from-navy-700 to-navy-900"
              ...
              >
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 gap-2">
                  <div className="w-10 h-10 rounded-full border-2 border-gold-500 flex items-center justify-center text-gold-500">
                    <FiZoomIn size={16} />
                  </div>
                  <span className="text-white text-xs font-sans bg-navy-900/70 px-3 py-1 rounded-full">
                    {item.label}
                  </span>
                </div>
              </motion.div>
      ...
      <AnimatePresence>
        {lightbox !== null && currentImg && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[60] bg-navy-950/95 backdrop-blur-sm flex items-center justify-center p-4"
            onClick={() => setLightbox(null)}
          >
            <button
              type="button"
              onClick={() => setLightbox(null)}
              className="absolute top-6 right-6 w-10 h-10 rounded-full border border-gold-500/30 text-gold-500 flex items-center justify-center hover:bg-gold-500/10 transition-colors z-10"
            >
              <FiX size={18} />
            </button>
            <button
              ...
              className="absolute left-4 md:left-8 w-10 h-10 rounded-full border border-gold-500/30 text-gold-500 flex items-center justify-center hover:bg-gold-500/10 transition-colors z-10"
            >
            ...
            <motion.div
              ...
              className="max-w-4xl w-full rounded-2xl overflow-hidden shadow-luxury"
            ...
            >
              ...
              <div className="bg-navy-900 px-4 py-2 text-center">
                <span className="text-gold-400 text-sm font-sans">{currentImg.label}</span>
              </div>
            </motion.div>
            <button
              type="button"
              onClick={e => { e.stopPropagation(); navigate(1); }}
              className="absolute right-4 md:right-8 w-10 h-10 rounded-full border border-gold-500/30 text-gold-500 flex items-center justify-center hover:bg-gold-500/10 transition-colors z-10"
            >
