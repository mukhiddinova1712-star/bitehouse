...
export default function Order() {
  ...
  const handleCheckout = () => {
    if (!address || !phone) {
      toast.error(locale === "en" ? "Enter address and phone!" : locale === "ru" ? "Введите адрес и телефон!" : "Manzil va telefon raqamni kiriting!", {
        style: { background: "#112052", color: "#f0f0f0" },
      });
      return;
    }
    if (isCardPayment && cardNumber.replace(/\s/g, "").length < 16) {
      toast.error(locale === "en" ? "Enter valid card number!" : locale === "ru" ? "Введите номер карты!" : "To'liq karta raqamini kiriting!", {
        style: { background: "#112052", color: "#f0f0f0" },
      });
      return;
    }
    ...
    toast.success(
      locale === "en" ? "Order placed! We will deliver soon." : locale === "ru" ? "Заказ принят! Доставим скоро." : "Buyurtmangiz qabul qilindi! Tez orada yetkazib beramiz.",
    ...
    <section id="order" className="section-padding bg-navy-950 relative overflow-hidden">
      ...
      <div className="container-custom" ref={ref}>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          >
            <h3 className="font-display text-xl text-gold-500 mb-6 flex items-center gap-2">
              ⭐ Mashhur taomlar
            </h3>
            <div className="grid sm:grid-cols-2 gap-4">
              {featured.map((item, i) => (
                <motion.div
                  ...
                  className="glass rounded-xl overflow-hidden flex items-center gap-3 p-3 group"
                >
                  ...
                  <button
                    onClick={() => { addToCart(item); toast.success(`${getName(item)} qo'shildi!`, { icon: "🛒", style: { background: "#112052", color: "#f0f0f0" } }); }}
                    className="w-8 h-8 rounded-full bg-gold-500/10 border border-gold-500/30 flex items-center justify-center text-gold-500 hover:bg-gold-500 hover:text-navy-900 transition-all flex-shrink-0"
                  >
                    <FiPlus size={14} />
                  </button>
                </motion.div>
          ...
          >
            <div className="glass-dark rounded-2xl p-6 sticky top-24">
              <h3 className="font-display text-xl text-gold-500 mb-4 flex items-center gap-2">
                ...
                {count > 0 && (
                  <span className="ml-auto w-6 h-6 bg-gold-500 text-navy-900 text-xs font-bold rounded-full flex items-center justify-center">
                    {count}
                  </span>
                )}
              </h3>
                ...
                <>
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              className="w-6 h-6 rounded-full border border-silver-700 text-silver-400 flex items-center justify-center hover:border-gold-500 hover:text-gold-500 transition-colors"
                            >
                              <FiMinus size={10} />
                            </button>
                            ...
                            <button
                              ...
                              className="w-6 h-6 rounded-full border border-red-900/50 text-red-400 flex items-center justify-center hover:border-red-500 transition-colors ml-1"
                            >
                              <FiTrash2 size={10} />
                            </button>
                          </div>
                  ...
                  <div className="space-y-3 mb-4">
                    <input
                      ...
                      placeholder={locale === "en" ? "Your name..." : locale === "ru" ? "Ваше имя..." : "Ismingiz..."}
                    />
                    <input
                      value={address}
                      onChange={e => setAddress(e.target.value)}
                      className="input-luxury w-full px-3 py-2 rounded-lg text-xs"
                      placeholder={locale === "en" ? "Delivery address..." : locale === "ru" ? "Адрес доставки..." : "Yetkazib berish manzili..."}
                    />
                    <input
                      value={phone}
                    ...
                    />
                    {/* Karta raqam - faqat onlayn to'lovda */}
                    <AnimatePresence>
                      {isCardPayment && (
                        <motion.div
                        ...
                        >
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="input-luxury w-full pl-8 pr-3 py-2 rounded-lg text-xs tracking-widest"
                  ...
                  {/* Payment */}
                  <div className="mb-4">
                    <p className="text-silver-400 text-xs font-sans tracking-wider uppercase mb-2">{t.order.payment}</p>
                    <div className="flex flex-wrap gap-2">
                      {paymentMethods.map(method => (
                        <button
                          key={method.id}
                          onClick={() => setPayment(method.id)}
                          className={`flex items-center gap-1 px-2 py-1 rounded text-xs font-sans transition-all
                            ${payment === method.id ? "bg-gold-500/20 border border-gold-500 text-gold-500" : "border border-silver-700/50 text-silver-500 hover:border-gold-500/50"}`}
                        >
                          <span>{method.icon}</span>
                          {t.order[method.label as keyof typeof t.order]}
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Totals */}
                  <div className="space-y-2 mb-4 pt-3 border-t border-silver-800/30">
                    <div className="flex justify-between text-silver-400 text-xs font-sans">
                      <span>{t.order.total}</span>
                      <span>{formatPrice(total)}</span>
                    </div>
                    <div className="flex justify-between text-silver-400 text-xs font-sans">
                      <span className="flex items-center gap-1"><FiTruck size={10} /> {t.order.delivery}</span>
                      <span>{formatPrice(deliveryFee)}</span>
                    </div>
                    <div className="flex justify-between text-gold-500 font-bold text-sm pt-1 border-t border-silver-800/30">
                      <span>Jami:</span>
                      <span>{formatPrice(grandTotal)}</span>
                    </div>
                  </div>

                  <button onClick={handleCheckout} className="btn-gold w-full py-3 rounded-xl text-xs tracking-widest">
                    {t.order.checkout}
                  </button>
                </>
              )}
