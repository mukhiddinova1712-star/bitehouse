...
export default function Order() {
  ...
  const handleCheckout = () => {
    ...
    if (isCardPayment && cardNumber.replace(/\s/g, "").length < 16) {
      toast.error(locale === "en" ? "Enter valid card number!" : locale === "ru" ? "Введите номер карты!" : "To'liq karta raqamini kiriting!", { style: { background: "#112052", color: "#f0f0f0" } });
      return;
    }
    const newOrder = {
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {paymentMethods.map(method => (
                        <button key={method.id} onClick={() => setPayment(method.id)} className={`flex items-center gap-1 px-2 py-1 rounded text-xs font-sans transition-all ${payment === method.id ? "bg-gold-500/20 border border-gold-500 text-gold-500" : "border border-silver-700/50 text-silver-500 hover:border-gold-500/50"}`}>
                          <span>{method.icon}</span>{t.order[method.label as keyof typeof t.order]}
                        </button>
                      ))}
                    </div>
                  </div>
                  <div className="space-y-2 mb-4 pt-3 border-t border-silver-800/30">
                    <div className="flex justify-between text-silver-400 text-xs font-sans"><span>{t.order.total}</span><span>{formatPrice(total)}</span></div>
                    <div className="flex justify-between text-silver-400 text-xs font-sans"><span className="flex items-center gap-1"><FiTruck size={10} /> {t.order.delivery}</span><span>{formatPrice(deliveryFee)}</span></div>
                    <div className="flex justify-between text-gold-500 font-bold text-sm pt-1 border-t border-silver-800/30"><span>Jami:</span><span>{formatPrice(grandTotal)}</span></div>
                  </div>
                  <button onClick={handleCheckout} className="btn-gold w-full py-3 rounded-xl text-xs tracking-widest">{t.order.checkout}</button>
                </>
              )}
            </div>
          </motion.div>
